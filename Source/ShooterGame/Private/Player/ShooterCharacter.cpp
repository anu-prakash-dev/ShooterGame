...
{
	const float HealthAmount = FMath::Min(1.0f, Health / GetMaxHealth());
	return HealthAmount;
}

int AShooterCharacter::GetKillsState() const
{
	AShooterPlayerController* MyPC = Cast<AShooterPlayerController>(Controller);
	...
	if (MyPlayerState)
	{
		return MyPlayerState->GetKills();
	}
	return 0;
}

int AShooterCharacter::GetCurrentWeaponAmmo() const
{
	if (CurrentWeapon)
	{
...
int AShooterCharacter::GetCurrentWeaponClip() const
{
	if (CurrentWeapon)
	{
